"""
build_neurips_notebook.py  (v3 — final, bug-free)
===================================================
Generates CogniSync_NeurIPS_Eval.ipynb.

All cell sources are stored in a plain list of strings so that no
triple-quote nesting or special-character issues arise.

Run:
    $env:PYTHONUTF8=1; python build_neurips_notebook.py
"""
import json
...
# ─────────────────────────────────────────────────────────────
# CELL SOURCES  (plain strings, no triple-quote nesting needed)
# ─────────────────────────────────────────────────────────────
...
SRC_DATASET = (
    ...
    "\n"
    "    # 5 turns: definition, noise, temporal override, paraphrase restate, final resolution\n"
    "    sess_texts = [\n"
    ...
    "    # Same topic/style; FAISS confuses them with GT; FTS+token-overlap resolves\n"
    "    dist_templates = [\n"
    ...
    "    ]\n"
    "    # --- HARD_LEX: exact-value disambiguation --- FTS decisive over FAISS ---\n"
    "    eval_queries += [\n"
    ...
    "\n"
    "BENCHMARK = {\n"
    "    'dataset_name': 'CogniSync NeurIPS Benchmark v2.1 (with distractors)', 'version': '2.1.0',\n"
    ...
    "diff = {d: sum(1 for q in eval_queries if q['difficulty']==d) for d in ('easy','medium','hard','hard_lex')}\n"
    "n_dist = sum(1 for s in sessions if 'distractor' in s.get('tags',[]))\n"
    "print(f'Benchmark v2.1 | {len(sessions)} sessions ({n_dist} distractors) | {len(eval_queries)} queries')\n"
...
SRC_SYSTEMS = (
    ...
    "    return r\n"
    "\n"
    "# ── CogniSync v2: Non-linear Adaptive Hybrid FTS5+FAISS ──────────────\n"
    "# Features: top_k=50, multiplicative fusion, ambiguity reranking, token overlap bonus\n"
    ...
    "        self.doc_ids=list(doc_ids); self.doc_map=dict(zip(doc_ids, docs)); t0=time.perf_counter()\n"
    ...
    "        # 1. FAISS Search -- expanded pool for disambiguation\n"
    ...
    "        }\n"
    "        \n"
    "        # 6. Non-linear fusion: s_sem * (1 + 0.4 * s_lex) -- lex amplifies sem only\n"
    "        final_scores = {}\n"
    "        for cid in candidate_ids:\n"
    "            s_sem = faiss_norm.get(cid, 0.0)\n"
    "            s_lex = fts_norm.get(cid, 0.0)\n"
    "            score = s_sem * (1.0 + 0.4 * s_lex)\n"
    "            if s_lex > 0.5: score += 0.07  # strong lexical bonus\n"
    "            final_scores[cid] = score\n"
    "        \n"
    "        # 7. Ambiguity reranking: when top-2 FAISS are close, boost lex for top-5\n"
...
# Assemble notebook
# ─────────────────────────────────────────────────────────────
md("""# CogniSync: NeurIPS-Level Comprehensive Evaluation Suite

**Authors:** Saurab Mishra, Shubham Mishra
*IISER Thiruvananthapuram | Capgemini India*

Paper: *"CogniSync: A Serverless Architecture for Persistent Agent Cognition via the Model Context Protocol"*

| Cell | Module | Key Outputs |
|------|--------|-------------|
| 1 | Environment | directories, helpers |
| 2 | Benchmark Dataset | JSON + 2 CSVs |
| 3 | System Definitions | metrics & all 8 retrieval classes |
| 4 | Retrieval Eval (8 systems x 5 seeds) | JSON + per-query CSV + aggregated CSV + stat-test CSV |
| 5 | Ablation Study (96 configs) | JSON + full CSV |
| 6 | Scalability (2K to 100K) | JSON + CSV |
| 7 | Security Eval | JSON + per-seed CSV + aggregated CSV |
| 8 | Agent Task Eval | JSON + per-query CSV + aggregated CSV |
| 9 | Figures (6 plots, PNG+PDF @ 300DPI) | 12 figure files |
| 10 | LaTeX Tables | 3 .tex files |
| 11 | Run Metadata + Dashboard | run_metadata.json |
| 12 | Download ZIP | cognisync_results.zip |

**Runtime:** ~45 min full / ~8 min FAST_MODE=True. No LLM API key needed.
""")

md("## Cell 1 -- Environment Setup")
code(SRC_ENV)

md("## Cell 2 -- CogniSync Benchmark Dataset v1.0")
code(SRC_DATASET)

md("## Cell 3 -- Metric Helpers + Retrieval System Definitions")
code(SRC_SYSTEMS)

md("## Cell 4 -- Retrieval Quality + Baselines (8 systems x 5 seeds)")
code(SRC_RETRIEVAL)

md("## Cell 5 -- Ablation Study (mode x Top-K x chunk-size x dedup)")
code(SRC_ABLATION)

md("## Cell 6 -- Scalability 2K to 100K")
code(SRC_SCALABILITY)

md("## Cell 7 -- Security & Adversarial Evaluation")
code(SRC_SECURITY)

md("## Cell 8 -- Agent Task Evaluation (Context-Sufficiency Oracle)")
code(SRC_AGENT)

md("## Cell 9 -- Publication-Ready Figures (6 plots, PNG + PDF @ 300 DPI)")
code(SRC_FIGURES)

md("## Cell 10 -- LaTeX Tables")
code(SRC_TABLES)

md("## Cell 11 -- Run Metadata & Summary Dashboard")
code(SRC_SUMMARY)

md("## Cell 12 -- Download All Results as ZIP")
code(SRC_ZIP)

# ─────────────────────────────────────────────────────────────
# Write output
# ─────────────────────────────────────────────────────────────
OUTPUT = Path(__file__).parent / "CogniSync_NeurIPS_Eval.ipynb"
